...
impl GetItemHandler {
    pub fn new(item_repo: Box<dyn ItemRepository + Send + Sync + 'static>) -> GetItemHandler {
        GetItemHandler { item_repo }
    }
}

#[async_trait]
impl Query<GetItem, Result<Item, ()>> for GetItemHandler {
    async fn handle(&self, query: GetItem) -> Result<Item, ()> {
...
mod tests {
    ...
    async fn it_should_get_item() {
        ...
        let handler = GetItemHandler::new(Box::new(InMemoryItemRepository::new(Arc::clone(
            &item_store,
        ))));
        let id = "id".to_string();
